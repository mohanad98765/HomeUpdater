...
| 📱 **Android** | ربط لاسلكي بمسح رمز QR أو بستّة أرقام — يجد المنفذ بالمسح، فلا يحتاج شبكةً تنقل mDNS |
| 🐧 **Linux** | تحديثات عبر SSH (apt / dnf) مع تحقّق هوية المضيف (TOFU) |
| 🏠 **Smart Home** | تكامل Home Assistant |
| 🌐 **فحص الشبكة** | ٢٥٤ عنوانًا في ٦٫٧ ثانية، وأسماء المصنّعين من سجلّ IEEE كاملًا (٣٩٣٧١ بادئة مضمّنة) |
| 🛡️ **الأمان** | مطابقة ثغرات NVD **بدقّة الإصدار** عبر CPE، مع المدى الذي جعل كلّ ثغرة تنطبق |
| 📄 **حزمة الدليل** | تقرير مبصوم بالهاش (HTML + CSV + JSON) بسلسلة تدقيق تكشف العبث، وتاريخ آخر استعلام من NVD، وتصريح بما **لم** يُفحص — بترخيص Ed25519 يعمل بلا إنترنت |
| 🛰️ **أجهزة أخرى (الوكيل)** | وكيل موقَّع Ed25519 على TLS مثبَّتة البصمة: يُبلّغ بتحديثاته، وتختار منها وتُثبّتها من شاشة واحدة — ولا يقبل معرِّفًا لم يُبلّغ عنه هو |
| 🔒 **قفل بكلمة مرور** | حماية بيانات التطبيق (PBKDF2 hashed، بلا افتراضي) |
| 🌍 **٦ لغات** | واجهة عربية أصيلة (RTL) + EN/FR/ES/TR/UR |

> **ما لا يفعله بعد** — مذكور هنا لأنّ إخفاءه يُكلّف أكثر: الوكيل **لا يُحدِّث نفسه**
> ولا يتّصل والجهاز على شاشة الدخول بلا مستخدم؛ والتوقيع **ذاتيّ** فقد يظهر تحذير
> SmartScreen على أجهزة أخرى؛ ومخرَج التقرير **HTML يُطبع** إلى PDF من المتصفّح — لا
> مولّد PDF داخل البرنامج، لأنّ PDF بخطّ عربيّ مكسور أسوأ من غيابه.
> التفصيل في [ما يُقال وما لا يُقال](03_الوثائق/ما_يُقال_وما_لا_يُقال.md).
